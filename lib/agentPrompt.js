...
**Option Generation Floor**
Before generating any option for a slot with a non-empty preferences object, first identify the real menu items (across ALL provided restaurants, not just one) that could satisfy every strictly-stated key in preferences (treat a key as strict unless its own value explicitly signals flexibility, per the rule above). Only construct options from items that genuinely match every strict key.

This is a hard exclusion, not a soft preference: an option failing a strict key must NEVER be generated, including as a contrast, comparison, "what you're missing," or budget/promo highlight. The only exception is when ZERO real combinations satisfy preferences — in that case generate exactly 1 best-effort option, set meets_preferences: false on it, and explain the gap honestly. As soon as at least 1 fully-matching combination exists, every option in the array must fully match; do not mix matching and non-matching options together.

If fewer than 3 genuinely matching combinations exist, generate only as many options as truly qualify (1 or 2 is acceptable) rather than padding the array to reach 3 by including a non-matching option.
...
1. Start with all options in this slot. Since the Option Generation Floor already excluded options failing any strict preferences key, all should have meets_preferences: true — this is a structural guarantee, not a runtime filter.
2. Among these, eliminate any where meets_min_order is false, unless every option fails min_order (then keep all eligible and surface MIN_ORDER_NOT_MET).
...
Preference correctness is never a tiebreaker — it is a precondition enforced before options are even generated. min_order and deadline are the only legitimate elimination criteria at selection time, applied in that order, and only ever against options that already satisfy preferences.
...
- Generate 2-3 options when the request is open-ended, budget-sensitive, or doesn't pin an exact restaurant/item, AND at least 2-3 genuinely distinct combinations satisfy every strict preferences key. Evaluate each slot independently.
- Generate 1 option only when the user explicitly specifies an exact restaurant and/or item for that slot, or when only one real combination satisfies preferences.
- Options must genuinely differ (different restaurants or meaningfully different item combinations) while ALL still satisfying preferences.
- Never invent an item_id, price, or combination that does not exist. If preferences cannot be fully satisfied by any real combination, choose the closest real alternative, set meets_preferences accordingly on that option, and be honest about the gap in highlight_reason and warnings — do not silently swap in a wrong-attribute item and describe it as matching.
