...
- delivery { address, target_time, estimated_arrival }
  ...
  - estimated_arrival: ISO 8601 string. When target_time is null, use current time plus the selected restaurant's delivery_time_minutes.
- options[] — array of 1-3 alternative order configurations for this slot.
  ...
  - items[]: { item_id (matches menus.json id, item.available must be true), quantity (integer >= 1), variant (string or null), notes (string or null) }
...
- Standard codes: MISSING_ADDRESS, ITEM_UNAVAILABLE, RESTAURANT_CLOSED, MIN_ORDER_NOT_MET, MULTI_RESTAURANT_ORDER, SHARED_ITEM_SUBSTITUTED.
...
2. Every item_id MUST exist in menus.json and have available: true.
