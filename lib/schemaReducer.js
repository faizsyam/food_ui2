...
function recalcLineTotal(item) {
  const basePrice = item.promo ? (item.promo.discounted_price ?? item.price) : item.price || 0;
...
function recalcSlotTiming(newSlot) {
  const selectedOption =
    newSlot.resolvedOptions?.find((o) => o.option_id === newSlot.selected_option_id) ||
    newSlot.resolvedOptions?.[0];
  if (!selectedOption) return;

  const prepTimes = (selectedOption.resolvedItems || [])
    .map((it) => it.preparation_time_minutes)
    .filter((v) => typeof v === 'number' && v > 0);

  const deliveryMinutes = selectedOption.restaurant?.delivery_time_minutes ?? 0;
  const maxPrep = prepTimes.length > 0 ? Math.max(...prepTimes) : 0;
  const sumPrep = prepTimes.reduce((a, b) => a + b, 0);

  newSlot.timing = {
    minMinutes: deliveryMinutes + maxPrep,
    maxMinutes: deliveryMinutes + sumPrep,
  };
}
...
function schemaReducer(schema, action) {
  ...
  switch (action.type) {
    ...
    case 'SELECT_OPTION': {
      const updatedSlots = (schema.slots || []).map((slot) => {
        if (slot.slot_id !== action.slot_id) return slot;
        const newSlot = { ...slot, selected_option_id: action.option_id };
        recalcSlotTiming(newSlot);
        return newSlot;
      });
      return { ...schema, slots: updatedSlots };
    }
